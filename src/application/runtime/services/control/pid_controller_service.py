...
def create_pid_controller(use_new: bool, logger):
    cls = PIDV2Controller if use_new else PIDController
    return cls(
        set_point=TARGET_CENTER_DISTANCE,
        kp=KP,
        ki=KI,
        kd=KD,
...
def check_and_update_pid(pid, last_pid_flag, shared_controls, logger):
    ...
    if current_pid_flag != last_pid_flag:
        pid = create_pid_controller(current_pid_flag, logger)
        logger.info(
            f"PID Controller atualizado para {'PID V2' if current_pid_flag else 'PID V1'}."
        )
