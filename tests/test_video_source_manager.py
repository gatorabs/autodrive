...
from src.infrastructure.adapters.video import video_manager_process as vmp
...
def test_open_video_source_warns_once_and_respects_cooldown(monkeypatch, capsys):
    ...
    monkeypatch.setattr(vmp, "VideoProcessor", mock_vp)

    times = [0.0]
    monkeypatch.setattr(vmp.time, "monotonic", lambda: times[0])
...
def test_success_resets_warning(monkeypatch, capsys):
    ...
    monkeypatch.setattr(vmp, "VideoProcessor", first)
    ...
    monkeypatch.setattr(vmp, "VideoProcessor", second)
